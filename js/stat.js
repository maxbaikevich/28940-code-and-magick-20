...
var GAP = 50;
var barHartHeight = -CLOUD_WIDTH + CLOUD_HEIGHT;
var CALOR_BLACK = '#000';
...
window.renderStatistics = function (ctx, nameArr, times) {
  ...
  for (var i = 0; i < nameArr.length; i++) {
    ...
    ctx.fillRect(
        ...
        BARC_HART_MARGIN,
        BARC_HART_WIDTH,
        (barHartHeight * times[i]) / maxTime
    );

    ctx.fillText(
        Math.floor(times[i]),
        LEFT_MARGIN + (GAP + BARC_HART_WIDTH) * i,
        (barHartHeight * times[i]) / maxTime + BARC_HART_MARGIN - 10
    );
  }
};
